...
def parse_number(number_str):
    ...
    Args: number_str (str): The formatted number string.

    Returns: int: The whole number without commas or decimal points.
    """
...
def remove_nan(string_value):
    ...
    if isinstance(string_value, str) and string_value.lower() == 'nan':
        ...
        return ''
    elif isinstance(string_value, str) and string_value.lower() == 'none':
        return ''
    else:
...
def combine_dicts(lst):
    ...
    prev_dict = None
    for d in lst:
        if d['prescribed ndc'] is not None and d['rowID'] is not None and d['prescribed ndc'] != 'None' and d['rowID'] != 'None':
            continue
        if all(value is None or value == 'None' for value in d.values()) and d['prescribed ndc'] is not None:
            if prev_dict is not None:
                prev_dict['prescribed ndc'] = d['prescribed ndc']
            else:
                prev_dict = d.copy()
        else:
            if prev_dict is not None:
                combined_list.append(prev_dict)
                prev_dict = None
            if not all(value is None or value == 'None' for value in d.values()) or d['prescribed ndc'] is not None:
...
def remove_trailing_spaces(dictionary):
    new_dict = {}
...
def clean_nan(string_value):
    ...
    if isinstance(string_value, str):
        ...
        cleaned_value = re.sub(r'None$', '', cleaned_value).strip()
            ...
            if folder.name == "tyson":
                ...
                for nfile in current_files: #process each file in current files one at a time
                    ...
                    if file_type == 'xlsx':
                        ...
                        for row in raw_data:
                            if row.get('dispensed ndc') != "" and row.get('dispensed ndc') != "nan" and row.get('dispensed ndc') != "None":
                                count_dNDC += 1
                        ...
                        for i, row in enumerate(raw_data):
                            raw_data[i] = remove_trailing_spaces(row)
                            for key, value in raw_data[i].items():
                                raw_data[i][key] = remove_nan(value)
                                raw_data[i][key] = clean_nan(value)
                            # print(raw_data[i])
                        ...
                        print(f"The Time it took to get {len(data)} rxcuis is {elapsed_time1}")
